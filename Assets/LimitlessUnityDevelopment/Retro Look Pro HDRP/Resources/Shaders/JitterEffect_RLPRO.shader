...
//#define PI 3.14159265359
	...
	float3 yiqDist(float2 uv, float m, float t) {
		...
		signal.x = rgb2yiq(SAMPLE_TEXTURE2D_X_LOD(_InputTexture, s_point_clamp_sampler, float2(offsetX.r, uv.y) , 0.0).rgb).x;
			//LOAD_TEXTURE2D_X(_InputTexture, float2(offsetX.r, uv.y) * _ScreenSize.xy).rgb).x;
		signal.y = rgb2yiq(SAMPLE_TEXTURE2D_X_LOD(_InputTexture, s_point_clamp_sampler, float2(offsetX.g, uv.y) , 0.0).rgb).y;
			//rgb2yiq(LOAD_TEXTURE2D_X(_InputTexture, float2(offsetX.g, uv.y) * _ScreenSize.xy).rgb).y;
		signal.z = rgb2yiq(SAMPLE_TEXTURE2D_X_LOD(_InputTexture, s_point_clamp_sampler, float2(offsetX.b, uv.y) , 0.0).rgb).z;
			//rgb2yiq(LOAD_TEXTURE2D_X(_InputTexture, float2(offsetX.b, uv.y) * _ScreenSize.xy).rgb).z;
		return signal;
	}
#endif
#if VHS_TWITCH_V_ON	
	...
	float4 Frag0(Varyings i) : SV_Target
	{
		...
		col = 
			SAMPLE_TEXTURE2D_X_LOD(_InputTexture, s_point_clamp_sampler, p , 0.0).rgb;
			//LOAD_TEXTURE2D_X(_InputTexture,i.texcoord +p * _ScreenSize.xy).rgb;
		signal = rgb2yiq(col);
#endif
